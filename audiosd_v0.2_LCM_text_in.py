...
def load_lcm_model(model_path, mode=0):
    ...
    if mode == 0:
        ...
        t1 = time.time()
        print("warmup----------Done, cost time(s): ", t1-t0)
...
def predict(mode, audio_input=None, user_input=None, param_template="None", height=512, width=512, steps=20, image_input=None, strength=0.5):
    ...
    if user_input:
        prompt_zh = user_input
        prompt_en = translate(prompt_zh)
        prompt = templates[param_template].format(prompt=prompt_en)
        yield prompt_zh, None, None
    ...
    if mode == 0: #txt2img   
        ...
        yield prompt_zh, output, sd_latency
    elif mode == 1: #img2img
...
if __name__ == '__main__':
    ...
    #with gr.Blocks(css=css) as demo: ## 可以在huging face下载模板
        ...
        with gr.Tab("文生图"):
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    sr_device = gr.Textbox(value="CPU",label="推理设备", visible=False)
                    sr_model_name = gr.Textbox(label="语音识别模型", value="whisper-medium", visible=False)
                    with gr.Row():
                    ...
                    with gr.Row():                   
                        sr_latency = gr.Textbox(label="语音识别耗时（s）", visible=False)
                ...
                with gr.Column(scale=8):
                    ...
                    user_input = gr.Textbox(show_label=False, placeholder="请在此输入文字...", lines=5, container=False, scale=3, interactive=True)
                    audio_input = gr.Audio(source="microphone", label="输入音频", type="filepath", interactive=True, visible=False)
                    with gr.Row():
        ...
        with gr.Tab("图生图"):
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    sr_device1 = gr.Textbox(value="CPU",label="推理设备", visible=False)
                    whisper_model1 = gr.Textbox(label="语音识别模型", value="whisper-medium", visible=False)
                    with gr.Column(scale=2):
                    ...
                    with gr.Column(scale=2):                   
                        sr_latency1 = gr.Textbox(label="语音识别耗时（s）", visible=False)
                ...
                with gr.Column(scale=8):
                    ...
                    with gr.Row():                                       
                        user_input1 = gr.Textbox(show_label=False, placeholder="请在此输入文字...", lines=5, container=False, scale=5,interactive=True)
                        audio_input1 = gr.Audio(source="microphone", label="输入音频", type="filepath", interactive=True, visible=False)
                    with gr.Row():
        ...
        submitBtn.click(predict, [mode, audio_input, user_input, param_template, height, width, steps],
                        [prompt_img, img_output, sd_latency], show_progress=True)
        ...
        submitBtn1.click(predict, [mode1, audio_input1, user_input1, param_template, height1, width1, steps1, img_input1, strength],
                        [prompt_img1, img_output1, sd_latency1], show_progress=True)
